...
function safeGetStageLabel(stage, fallback) {
  try {
    ...
    console.error('getStageLabel 호출 오류:', error);
    return fallback || stage || '-';
  }
}
...
function populateRoundsWithClues(
  ...
  rounds.forEach((round, index) => {
    ...
    const stageLabel = safeGetStageLabel(round.stage, round.label || "단계");
...
async function transitionToStage(stageKey, options = {}) {
  ...
  if (!options.silent) {
    showToast(`현재 단계가 '${safeGetStageLabel(stageKey)}'(으)로 전환되었습니다.`, "info");
  }
  startStageTimerLoop();
  return updated;
}
...
async function handleStageUpdate() {
  ...
  try {
    ...
    showToast(`현재 단계가 '${safeGetStageLabel(stageKey)}'(으)로 변경되었습니다.`, "success");
  } catch (error) {
    console.error(error);
...
function formatReadyStatus(player) {
  ...
  if (player.ready_stage) {
    return `준비 (${safeGetStageLabel(player.ready_stage)})`;
  }
  return "준비";
}
...
async function checkAndHandleStageReadyAdvance() {
  ...
  try {
    await transitionToStage(nextStage, { silent: true });
    showToast(
      `턴 끝내기 투표가 충족되어 '${safeGetStageLabel(stage)}' 단계를 종료합니다.`,
      "info"
    );
    await loadPlayers();
  } catch (error) {
